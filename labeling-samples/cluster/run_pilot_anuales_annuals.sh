...
python scripts/02_generate_labels_c2_cluster.py \
  ...
  --labels-dir /home/lserey/mapbiomas_land/tmp/labels/annuals \
  --only-groups anuales \
  --max-rows 5 \
  --overwrite \
  --export-gee-asset \
  --gee-wait \
  --gee-asset-base projects/mapbiomas-chile/assets/LULC/COLLECTION-03/SAMPLES/SAMPLES_COL3/SAMPLES_SSL4EO
